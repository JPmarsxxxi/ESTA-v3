...
// Every effect shader shares one uniform block (resolution, direction,
// scalars); each declares which uniforms fill `scalars`, in order, and whether
// it takes `u_direction`.
struct ShaderSpec {
    id: &'static str,
    source: &'static str,
...
impl EffectPipeline {
    pub fn new(context: &GpuContext) -> Self {
        ...
        let build_pipeline = |spec: &ShaderSpec| {
            let shader_module =
                context
                    .device()
                    .create_shader_module(wgpu::ShaderModuleDescriptor {
...
fn pack_effect_uniforms(
    ...
    let spec = SHADERS
        .iter()
        .find(|spec| spec.id == shader)
        .ok_or_else(|| EffectsError::UnknownEffectShader {
            shader: shader.to_string(),
        })?;
    let mut scalars = [0.0; 4];
    for (slot, name) in scalars.iter_mut().zip(spec.scalars) {
        *slot = read_number_uniform(pass, name)?;
    }
    let direction = if spec.direction {
    ...
    for uniform in pass.uniforms.keys() {
        if spec.scalars.contains(&uniform.as_str()) || (spec.direction && uniform == "u_direction")
        {
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn packs_blur_as_before() {
        let packed = pack_effect_uniforms(
            &pass(
                "gaussian-blur",
                &[
                    ("u_sigma", UniformValue::Number(3.0)),
        ...
        .unwrap();
        assert_eq!(packed.resolution, [100.0, 50.0]);
        assert_eq!(packed.direction, [1.0, 0.0]);
        assert_eq!(packed.scalars, [3.0, 1.0, 0.0, 0.0]);
    }

    #[test]
    fn packs_color_adjust_in_declared_order() {
        let packed = pack_effect_uniforms(
            &pass(
                "color-adjust",
                &[
                    ("u_hue", UniformValue::Number(30.0)),
                    ("u_brightness", UniformValue::Number(0.1)),
                    ("u_saturation", UniformValue::Number(-0.5)),
                    ("u_contrast", UniformValue::Number(0.2)),
                ],
            ),
            10,
            10,
    ...
    #[test]
    fn rejects_unknown_shader_and_uniform() {
        assert!(matches!(
        ...
        let extra = pass(
            "color-adjust",
            &[
                ("u_brightness", UniformValue::Number(0.0)),
                ("u_contrast", UniformValue::Number(0.0)),
                ("u_saturation", UniformValue::Number(0.0)),
                ("u_hue", UniformValue::Number(0.0)),
                ("u_direction", UniformValue::Vector(vec![1.0, 0.0])),
            ],
        );
        assert!(matches!(
            pack_effect_uniforms(&extra, 1, 1),
